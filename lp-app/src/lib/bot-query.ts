...
const EXEC_PATTERNS: { pattern: RegExp; scriptId: string; label: string }[] = [
  // V2 pipeline (primary — matches "パイプライン実行", "V2実行", "全自動", etc.)
  ...
  { pattern: new RegExp(`パイプライン.*(${ACTION_VERBS})`), scriptId: "orchestrate_v2", label: "V2パイプライン" },
  { pattern: new RegExp(`(${ACTION_VERBS}).*パイプライン`), scriptId: "orchestrate_v2", label: "V2パイプライン" },
  { pattern: new RegExp(`(全自動|フル|一気通貫|エンドツーエンド|e2e).*(${ACTION_VERBS})`), scriptId: "orchestrate_v2", label: "V2パイプライン" },
  { pattern: new RegExp(`(${ACTION_VERBS}).*(全自動|フル|一気通貫|エンドツーエンド|e2e)`), scriptId: "orchestrate_v2", label: "V2パイプライン" },
  { pattern: new RegExp(`市場(調査|リサーチ).*(${ACTION_VERBS})`), scriptId: "orchestrate_v2", label: "V2パイプライン" },
  { pattern: new RegExp(`(${ACTION_VERBS}).*市場(調査|リサーチ)`), scriptId: "orchestrate_v2", label: "V2パイプライン" },
  { pattern: new RegExp(`競合(調査|分析).*(${ACTION_VERBS})`), scriptId: "orchestrate_v2", label: "V2パイプライン" },
  { pattern: new RegExp(`(${ACTION_VERBS}).*競合(調査|分析)`), scriptId: "orchestrate_v2", label: "V2パイプライン" },
  { pattern: new RegExp(`事業案.*(生成|作って|${ACTION_VERBS})`), scriptId: "orchestrate_v2", label: "V2パイプライン" },
  // Downstream execution jobs (still active independently)
  ...
  // Blog generation
  { pattern: new RegExp(`ブログ.*(生成|作って|${ACTION_VERBS})`), scriptId: "blog_generator", label: "ブログ記事生成" },
  { pattern: new RegExp(`(${ACTION_VERBS}).*ブログ`), scriptId: "blog_generator", label: "ブログ記事生成" },
  { pattern: new RegExp(`記事.*(生成|作って|${ACTION_VERBS})`), scriptId: "blog_generator", label: "ブログ記事生成" },
  // SNS batch generation
  { pattern: new RegExp(`sns.*(バッチ|一括|プリ生成|まとめて).*(${ACTION_VERBS}|生成)`), scriptId: "sns_batch_generator", label: "SNSバッチ生成" },
  { pattern: new RegExp(`(バッチ|一括).*(sns|投稿).*(${ACTION_VERBS}|生成)`), scriptId: "sns_batch_generator", label: "SNSバッチ生成" },
...
# ★ フロー説明時の絶対ルール ★
「フロー」「全体像」「仕組み」を聞かれたら、**V2パイプライン → LP生成 → ブログ50記事 → SNSバッチ生成 → SNSスケジュール投稿 → 公開サイト** の**全ステップ**を必ず説明すること。
LP作成ガードで説明を止めてはならない。LP作成ガード以降のLP生成・ブログ・SNS・公開サイトは本システムの重要な構成要素であり、省略は禁止。

# BVA System の仕組み（実装済み・本番稼働中）

## ★★ 最重要: V2パイプライン + LP + ブログ + SNS + 公開サイト（本番）★★
...
- **O(即決オファー3案)**: 穴を埋める即決オファー。7必須フィールド: payer, offer_name, deliverable, time_to_value, price, replaces, upsell → offer_3_logシート
- **LP作成ガード**: ゲートPASS + 競合10社以上 + オファー3案完備 → lp_ready_logシート
- **LP生成**: lp_ready_logがREADYの事業に対してLPページを自動生成 → Supabase + Vercel
- **ブログ50記事生成**: LP生成完了後に自動トリガー。市場×ペルソナに特化したSEO記事を50本生成 → blog_articlesシート + Supabase posts
- **SNSバッチ生成**: 10カテゴリ×10投稿=100投稿をプリ生成 → sns_queueシート
...
- sns-poster: リアルタイムSNS投稿（2_sns_poster）
- sns-batch-generator: SNS100投稿バッチ生成
- sns-scheduled-poster: sns_queueから毎日自動投稿
...
function getSystemExplanation(): string {
  return (
    ...
    `📊 ダッシュボード: https://shokunin-san.xyz/dashboard`
  );
}
