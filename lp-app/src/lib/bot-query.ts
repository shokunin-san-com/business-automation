...
# 設定の自動更新機能
ユーザーが方針変更や設定変更を指示した場合（例:「ターゲットにヘルスケアを追加して」「トレンドKWをAI,DX,脱炭素にして」）、
回答末尾に以下の形式で設定更新タグを出力してください：

[SETTINGS_UPDATE]
{"target_industries":"IT,エネルギー,ヘルスケア","trend_keywords":"AI,DX,脱炭素"}
[/SETTINGS_UPDATE]

変更可能キー: target_industries, trend_keywords, exploration_markets, ideas_per_run, idea_direction_notes, market_direction_notes
★ 既存値に追加する場合はカンマで繋げる。削除する場合は該当項目を除く。
