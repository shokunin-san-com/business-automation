...
def save_research_to_sheets(research_list: list, batch_id: str) -> int:
    ...
    for r in research_list:
        ...
        rows.append([
            ...
            r.get("regulations", ""),
            json.dumps(r.get("data_sources", []), ensure_ascii=False) if isinstance(r.get("data_sources"), list) else str(r.get("data_sources", "Claude知識ベース")),
            r.get("confidence_score", 3),
            r.get("confidence_rationale", ""),
            "draft",
            batch_id,
            now,
        ])
